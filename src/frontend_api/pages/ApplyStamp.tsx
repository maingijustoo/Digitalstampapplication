...
import { applicantApi, applicationApi, stampTypeApi } from "../../frontend_api/api";
...
// The stamp type code used for KCPP business verification applications
const KCPP_STAMP_CODE = "CERT-GOOD";

export function ApplyStamp() {
  const [step, setStep]           = useState(0);
  const [submitting, setSubmitting] = useState(false);
  const [submitError, setSubmitError] = useState<string | null>(null);
  const [result, setResult]       = useState<{ referenceNumber: string; businessName: string } | null>(null);

  const [form, setForm] = useState<AppForm>({
    businessName: "", businessHandle: "", website: "", category: "",
    yearEstablished: "", regNumber: "", location: "", county: "",
    contactName: "", contactEmail: "", contactPhone: "", description: "",
    mpesaPaybill: "", hasPhysicalAddress: false,
    agreedToTerms: false, agreedToAudit: false,
  });
  ...
  // ── SUBMIT → posts to Django backend ──────────────────────────────────────
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Create or find the applicant (contact person)
      const applicant = await applicantApi.create({
        full_name:      form.contactName,
        email:          form.contactEmail,
        phone:          form.contactPhone,
        applicant_type: "company",
        id_number:      form.regNumber || "",
        address:        `${form.county}, Kenya`,
      });

      // 2. Find the default KCPP stamp type
      const stampTypesRes = await stampTypeApi.list(true);
      const stampType = stampTypesRes.results.find(
        (st) => st.code === KCPP_STAMP_CODE
      ) ?? stampTypesRes.results[0];

      if (!stampType) throw new Error("No stamp types available. Please run seed_data on the server.");

      // 3. Create the application
      const application = await applicationApi.create({
        applicant:   applicant.id,
        stamp_type:  stampType.id,
        purpose:     `KCPP Digital Trust Stamp application for ${form.businessName}. ` +
                     `Category: ${form.category}. County: ${form.county}. ` +
                     `${form.description}`,
        notes:       [
          form.businessHandle  ? `Handle: ${form.businessHandle}`           : "",
          form.website         ? `Website: ${form.website}`                 : "",
          form.mpesaPaybill    ? `M-Pesa Paybill/Till: ${form.mpesaPaybill}` : "",
          form.regNumber       ? `Reg Number: ${form.regNumber}`            : "",
          `Year Established: ${form.yearEstablished}`,
          `Physical Address: ${form.hasPhysicalAddress ? "Yes" : "No"}`,
        ].filter(Boolean).join(" | "),
        priority:    "normal",
        fee_amount:  parseFloat(stampType.fee),
      });

      if (!application.id) {
    throw new Error('Application was created but no ID was returned. Check serializer.');
      }
      // 4. Auto-submit the application (draft → submitted)
      await applicationApi.submit(application.id, form.contactName);
      ...
      <div className="max-w-2xl mx-auto px-4 py-12 text-center">
        <div className="rounded-xl p-8 shadow-lg" style={{ background: "#fff", border: "2px solid #2ecc71" }}>
          ...
          <p className="mt-3" style={{ color: "#6b7280", fontSize: "0.9rem" }}>
            Your application for a KCPP Digital Trust Stamp has been received. Our team will review
            your submission within 5–10 business days.
          </p>
          <div className="mt-5 p-4 rounded-lg text-left" style={{ background: "#f8f9fc" }}>
            <p style={{ color: "#1a2f5e", fontWeight: 700, fontSize: "0.85rem" }}>
              Application Reference: {result.referenceNumber}
            </p>
            <p className="mt-1" style={{ color: "#9aa3b5", fontSize: "0.8rem" }}>Business: {result.businessName}</p>
            <p style={{ color: "#9aa3b5", fontSize: "0.8rem" }}>Status: Submitted — Pending Review</p>
          </div>
    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        {[
          ...
          <div key={b.label} className="flex flex-col items-center p-3 rounded-lg text-center"
            ...
            <div className="flex flex-col items-center">
              <div className="w-9 h-9 rounded-full flex items-center justify-center"
                style={{
                  ...
                  color: i <= step ? "#fff" : "#9aa3b5", fontWeight: 700,
                }}>
      ...
      {/* API error banner */}
      {submitError && (
        <div className="mb-4 p-3 rounded-lg text-sm" style={{ background: "#fce4ec", color: "#c0392b", border: "1px solid #e74c3c" }}>
          ⚠️ {submitError}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit}>
        <div className="rounded-xl shadow p-6" style={{ background: "#fff", border: "1px solid #e0e4ed" }}>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  { name: "businessHandle", label: "Social Media Handle",     placeholder: "@YourHandle" },
                  { name: "website",        label: "Website (if any)",        placeholder: "yoursite.co.ke" },
                  { name: "regNumber",      label: "Business Reg. Number",    placeholder: "CPR/2019/XXXXXX" },
                  { name: "mpesaPaybill",   label: "M-Pesa Paybill / Till",   placeholder: "e.g., 123456" },
                ].map(({ name, label, placeholder }) => (
                  <div key={name}>
                    <label className="block mb-1" style={{ fontSize: "0.82rem", fontWeight: 600, color: "#1a2f5e" }}>{label}</label>
                    <input name={name} value={(form as any)[name]} onChange={handleChange} placeholder={placeholder}
                      className="w-full px-3 py-2 rounded-lg outline-none"
                      style={{ border: `1px solid ${(errors as any)[name] ? "#e74c3c" : "#e0e4ed"}`, fontSize: "0.88rem" }} />
                    {(errors as any)[name] && <p style={{ color: "#e74c3c", fontSize: "0.72rem" }} className="mt-1">{(errors as any)[name]}</p>}
                  </div>
                ))}

                <div>
                  <label className="block mb-1" style={{ fontSize: "0.82rem", fontWeight: 600, color: "#1a2f5e" }}>Year Established *</label>
                  <input type="number" name="yearEstablished" value={form.yearEstablished} onChange={handleChange}
                    placeholder="e.g., 2019" min="1900" max="2026" className="w-full px-3 py-2 rounded-lg outline-none"
                    style={{ border: `1px solid ${errors.yearEstablished ? "#e74c3c" : "#e0e4ed"}`, fontSize: "0.88rem" }} />
                ...
                <div>
                  ...
                  <select name="category" value={form.category} onChange={handleChange} className="w-full px-3 py-2 rounded-lg outline-none"
                    style={{ border: `1px solid ${errors.category ? "#e74c3c" : "#e0e4ed"}`, fontSize: "0.88rem", background: "#fff" }}>
                ...
                <div>
                  ...
                  <select name="county" value={form.county} onChange={handleChange} className="w-full px-3 py-2 rounded-lg outline-none"
                    style={{ border: `1px solid ${errors.county ? "#e74c3c" : "#e0e4ed"}`, fontSize: "0.88rem", background: "#fff" }}>
              ...
              <div>
                ...
                <textarea name="description" value={form.description} onChange={handleChange} rows={4} maxLength={400}
                  placeholder="Describe your business, products/services, target market, and why you're applying..."
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" name="hasPhysicalAddress" id="hasPhysicalAddress" checked={form.hasPhysicalAddress}
          ...
          {step === 1 && (
            <div className="flex flex-col gap-4">
              ...
              <p style={{ color: "#6b7280", fontSize: "0.82rem" }}>
                Used for verification communication only. Not publicly displayed.
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  { name: "contactName",  label: "Full Name *",        type: "text",  placeholder: "Jane Wanjiru" },
                  { name: "contactEmail", label: "Email Address *",     type: "email", placeholder: "jane@yourbusiness.co.ke" },
                  { name: "contactPhone", label: "Phone Number *",      type: "text",  placeholder: "+254 7XX XXX XXX" },
              ...
              <div className="p-4 rounded-lg" style={{ background: "#e8f5e9", border: "1px solid #2ecc71" }}>
                ...
                <p style={{ color: "#1a4d2e", fontSize: "0.82rem" }}>
                  The KCPP Digital Stamp annual verification fee is <strong>KES 5,000</strong> for SMEs and{" "}
                  <strong>KES 15,000</strong> for enterprises. Payment instructions will be sent to your email upon approval.
                </p>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-start gap-2">
                <input type="checkbox" name="agreedToTerms" id="agreedToTerms" checked={form.agreedToTerms}
                  onChange={handleChange} style={{ width: 18, height: 18, marginTop: 2 }} />
                <label htmlFor="agreedToTerms" style={{ fontSize: "0.8rem", color: "#4b5563" }}>
                  I agree to KCPP's Terms of Service and Privacy Policy. I confirm all information provided is accurate
                  and I am authorized to represent this business. *
                </label>
              </div>
              {errors.agreedToTerms && <p style={{ color: "#e74c3c", fontSize: "0.72rem" }}>{errors.agreedToTerms}</p>}

              <div className="flex items-start gap-2">
                <input type="checkbox" name="agreedToAudit" id="agreedToAudit" checked={form.agreedToAudit}
                  onChange={handleChange} style={{ width: 18, height: 18, marginTop: 2 }} />
                <label htmlFor="agreedToAudit" style={{ fontSize: "0.8rem", color: "#4b5563" }}>
                  I agree to cooperate with KCPP's verification audit process, which may include providing additional
                  documentation, business registration proof, and/or site visits. *
                </label>
              </div>
          ...
          <div className="flex gap-3 mt-6">
            {step > 0 && (
              <button type="button" onClick={() => setStep(step - 1)} className="flex-1 py-3 rounded-lg"
                style={{ background: "#f4f6fb", color: "#1a2f5e", fontWeight: 700, border: "1px solid #e0e4ed" }}>
                ← Back
              </button>
            )}
            {step < 2 ? (
              <button type="button" onClick={handleNext} className="flex-1 py-3 rounded-lg"
                style={{ background: "#1a2f5e", color: "#fff", fontWeight: 700 }}>
                Continue →
              </button>
            ) : (
              <button type="submit" disabled={submitting} className="flex-1 py-3 rounded-lg"
                style={{ background: submitting ? "#b8940f" : "#d4a017", color: "#fff", fontWeight: 700, cursor: submitting ? "not-allowed" : "pointer" }}>
