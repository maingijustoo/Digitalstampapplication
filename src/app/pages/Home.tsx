...
import { Search, Shield, Flag, Bell, QrCode, ChevronRight, CheckCircle, AlertTriangle, XCircle } from "lucide-react";
import { TrustBadge } from "../components/TrustBadge";
import { businesses, fraudReports, scamAlerts } from "../data/mockData";
import exampleImage from "figma:asset/c8a95ca596d5865b03e368678081f90b7cb8b010.png";
...
export function Home() {
  ...
  const recentlyVerified = businesses.filter((b) => b.badge === "verified").slice(0, 4);
  const recentReports = fraudReports.slice(0, 5);
  const topScams = scamAlerts.slice(0, 4);
      ...
      <section className="relative overflow-hidden" style={{ background: "#1a2f5e", minHeight: 340 }}>
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 py-12 text-center">
          ...
          <p className="text-white/60 mb-8" style={{ fontSize: "0.9rem" }}>
            Kenyan 'Consumer Protection Platform' for digital commerce fraud prevention
          </p>
          <form onSubmit={handleSearch} className="flex max-w-xl mx-auto">
            ...
            <button
              type="submit"
              className="px-5 py-3 rounded-r-lg flex items-center justify-center transition-colors"
          ...
          {[
            {
              ...
              desc: "Check if a business is KCPP verified before transacting",
              color: "#e8f5e9",
              border: "#2ecc71",
            },
            {
              ...
              icon: "🚨",
              label: "Report Fraud",
              desc: "Report a suspicious business or scam to protect others",
              color: "#fff3e0",
              border: "#e67e22",
            },
            {
              ...
              desc: "Stay updated on the latest fraud schemes in Kenya",
              color: "#fce4ec",
              border: "#e74c3c",
            },
          ].map((card) => (
      ...
      <section className="max-w-5xl mx-auto px-4 mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            {recentlyVerified.map((biz) => (
              <Link
                key={biz.id}
                to={`/verify?q=${encodeURIComponent(biz.handle)}`}
              ...
              >
                ...
                <p className="mt-3" style={{ fontWeight: 700, fontSize: "0.9rem", color: "#1a2f5e" }}>
                  {biz.name}
                </p>
                <p style={{ fontSize: "0.72rem", color: "#9aa3b5" }}>KCPP VERIFIED [YEAR]</p>
              </Link>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {businesses.filter((b) => b.badge === "at-risk" || b.badge === "flagged").slice(0, 2).map((biz) => (
              <Link
                key={biz.id}
                to={`/verify?q=${encodeURIComponent(biz.handle)}`}
                className="flex flex-col items-center p-4 rounded-xl shadow hover:shadow-md transition-all"
              ...
              >
                ...
                <p style={{ fontSize: "0.72rem", color: "#9aa3b5" }}>
                  {biz.badge === "flagged" ? "⚠️ DO NOT TRANSACT" : "KCPP MONITORED [YEAR]"}
                </p>
              </Link>
            ))}
          </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-xl shadow p-4" style={{ background: "#fff", border: "1px solid #e0e4ed" }}>
            ...
            {[
              { num: "1.", label: "VERIFY A BUSINESS", sub: "Scan QR codes (concept)", to: "/verify", icon: <QrCode className="w-5 h-5" /> },
              { num: "2.", label: "REPORT A FRAUD", sub: null, to: "/report-fraud", icon: <Flag className="w-5 h-5" /> },
              { num: "3.", label: "MY REPORTS", sub: null, to: "/business-portal", icon: <Bell className="w-5 h-5" /> },
            ].map((item) => (
              <Link
                key={item.to}
                to={item.to}
                ...
                {topScams.map((scam) => (
                  <tr key={scam.id} style={{ borderTop: "1px solid #f0f2f6" }}>
                    <td className="py-2 pr-2" style={{ color: "#1a2f5e", fontWeight: 600, borderLeft: "3px solid #e74c3c", paddingLeft: 6 }}>
                      <div>{new Date(scam.date).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "2-digit" })}</div>
                    </td>
            ...
            {recentReports.map((report) => (
              ...
              >
                <div
                  ...
                  style={{ background: "#e0e4ed" }}
                >
                  <span style={{ fontWeight: 700, fontSize: "0.8rem", color: "#1a2f5e" }}>@</span>
                </div>
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span style={{ fontWeight: 700, fontSize: "0.82rem", color: "#1a2f5e" }}>
                      {report.business}
                    </span>
                    <span
                      ...
                      style={{
                        ...
                        fontWeight: 600,
                        background:
                          report.severity === "high" ? "#fce4ec" : report.severity === "medium" ? "#fff3e0" : "#e8f5e9",
                        color:
                          report.severity === "high" ? "#c0392b" : report.severity === "medium" ? "#e67e22" : "#27ae60",
                      }}
                    >
                ...
                <span
                  ...
                  style={{
                    ...
                    fontWeight: 600,
                    background:
                      report.status === "resolved" ? "#e8f5e9" : report.status === "investigating" ? "#e3f2fd" : "#fff3e0",
                    color:
                      report.status === "resolved" ? "#27ae60" : report.status === "investigating" ? "#1565c0" : "#e67e22",
                  }}
                >
